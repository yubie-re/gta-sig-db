<script lang="ts">
    ...
    let rtmaHandler = new DataHandler<ProcessedSignature>([], {
        rowsPerPage: 10,
    });
    rtmaHandler.sortDesc("time");
    let intgHandler = new DataHandler<ProcessedSignature>([], {
        rowsPerPage: 10,
    });
    ...
    onMount(async () => {
        try {
            const response = await fetch(
                "https://raw.githubusercontent.com/yubie-re/gta-sig-db/main/sig-workflow/sig_db.json",
            );
            const data: { RTMA: SignatureData[]; INTG: SignatureData[] } =
                await response.json();
...
<svelte:head>
    <title
        >GTA 5 Anticheat Signature Database - View Anticheat Data & Research</title
    >
    <meta
        name="description"
    ...
    <meta
        property="og:title"
        content="GTA 5 Anticheat Signature Database - View Anticheat Data & Research"
    />
    <meta
        ...
        <AppBar>
            ...
            <svelte:fragment slot="trail"
                ><a href="https://github.com/yubie-re/gta-sig-db"
                    ><GithubIcon
                        class="text-2xl"
                        href="https://github.com/yubie-re/gta-sig-db"
                    /></a
                ></svelte:fragment
            >
        </AppBar>
            ...
            <div class="space-y-5">
                <div class="card p-5 bg-primary-300">
                    <p>
                        ...
                        Translations of signature hashes are provided when
                        possible.
                    </p>
                </div>
